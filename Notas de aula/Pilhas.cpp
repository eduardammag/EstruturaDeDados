...
// - Pilha (Stack):
// Segue o princípio LIFO (Last In, First Out), ou seja, o último elemento adicionado à pilha é o primeiro a ser removido.

// - Node:
// É uma estrutura de dados básica que contém um valor de dados (iData) e um ponteiro para o próximo nó na sequência.
...
Stack* newStack()
{   
    Stack* temp = (Stack*) malloc(sizeof(Stack)); // Aloca memória para uma nova pilha
...
void pop(Stack* const stack)
{
    ...
    if (stack->ptrTop == nullptr) // Verifica se a pilha está vazia
    {
        // Se a pilha estiver vazia, imprime uma mensagem indicando isso
        cout << "Pilha vazia" << endl;
        return; // Encerra o if
